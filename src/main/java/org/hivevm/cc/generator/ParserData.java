  ...
  public final boolean genFirstSet(Expansion exp, boolean[] firstSet, boolean jj2la) {
    if (exp instanceof RExpression re) {
      ...
      for (Expansion element : ch.getChoices()) {
        jj2la = genFirstSet(element, firstSet, jj2la);
      }
    }
    else if (exp instanceof Sequence seq) {
      Object obj = seq.getUnits().getFirst();
      if ((obj instanceof Lookahead) && (!((Lookahead) obj).getActionTokens().isEmpty())) {
        jj2la = true;
      }
    ...
    else if (exp instanceof ZeroOrOne zo) {
