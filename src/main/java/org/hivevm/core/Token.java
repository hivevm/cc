...
/**
 * Describes an input token. This is the canonical, language-independent token type used across
 * HiveVM CC's own front end (parse model, semantic analysis, generators) so that the model does not
 * depend on a generated parser class (ADR-0013). It mirrors the fields of the token boilerplate that
 * HiveVM CC emits for users' parsers ({@code templates/java/Token.java}); users' generated parsers
 * keep their own self-contained {@code Token} and never see this class.
 */
public class Token {

    /**
     * An integer that describes the kind of this token. This numbering system is determined by the
     * parser generator, and a table of these numbers is stored in the {@code ...Constants} file.
     */
    public int kind;
